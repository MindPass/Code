...
if id_message not in liste_id:
    ...
    if rootMessage.is_multipart():
        try:
            corps = rootMessage.get_payload(0).decode('utf-8-sig')
            print(corps)
        ...
        try:
            corps = rootMessage.get_payload().decode('utf-8-sig')
            encodages = re.findall(r'charset=[a-zA-Z0-9-]+', corps)
            encodage = encodages[0].split("=")[1]
            print(encodage)
            print(corps)
    ...
    cur.execute("INSERT INTO mails (id, expediteur, sujet, contenu, date) VALUES(?,?,?,?,?)",
                (id_message, exp, subject, corps, date))
