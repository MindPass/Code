...
class TableExterne(object):
    ...
    def raw_email(self, email_id):
        ...
        try:
            ...
            print('Décodage de raw_email NON effectué: '+str(e))

    def print_raw_email(self):
        result, data = self.imap_conn.fetch(email_id, "(RFC822)")
        raw_email = data[0][1]
    ...
    def email_as_list(self, email_id):
        ...
        if (rootMessage.is_multipart()):
            try:
                corps = rootMessage.get_payload(0, decode=True)
            except Exception as e:
                print("Multipart: "+str(e))
        # Récupérer le corps du mail en plain/text bien décodé
        else:
            try:
                corps = rootMessage.get_payload(decode=True)
                print(corps)
